...
// Use Catch's value for console display_width (store Clara's off to the side, if present)
...
// Restore Clara's value for console display_width, if present
...
void ConsoleReporter::benchmarkPreparing(std::string const& name) {
	...
	auto nameCol = Column(name).width(static_cast<std::size_t>(m_tablePrinter->columnInfos()[0].display_width - 2));
